...
class PrefMetrics:
    def __init__(
        ...
        self.state = {
            ...
            "preference_accuracy": deque(maxlen=window_size),
            "step_time": deque(maxlen=window_size),
            "lr": [],
        }

        #self.dataset_trackers = defaultdict(lambda: PrefMetrics([], "", "", {}))
    ...
    def push(self) -> str:
        ...
        preference_accuracy = torch.stack(list(self.state["preference_accuracy"])).mean().item()
        step_time, lr = np.mean(list(self.state["step_time"])), self.state["lr"][-1]
        status = self.get_status(loss)
        ...
        #            f"{ds}/Preference Token Accuracy": torch.stack(list(tracker.state["preference_accuracy"])).mean().item(),
        #        }
        #    )
        ...
        self.log(
            ...
            metrics={
                ...
                f"{prefix}/Preference Token Accuracy": preference_accuracy,
                f"{prefix}/Loss (Raw)": loss_raw,
                f"{prefix}/Learning Rate": lr,
                f"{prefix}/Step Time": step_time,
            },
